  ...
  <div>
    <el-header class="main-header">
      <div class="filter-bar">
        <el-select v-model="typeFilter" placeholder="选择类型" clearable>
        ...
        <el-table :data="filteredNewsList" class="news-table row-grey" style="width: 100%" height="calc(100vh - 160px)"
          ...
          <el-table-column prop="id" label="ID" width="50">
            <template v-slot="scope">
              <span :class="{ strikethrough: scope.row.send === 1 }">{{
                scope.row.id
                }}</span>
            </template>
          </el-table-column>
    ...
    <div v-if="publishDialogVisible" class="publish-platforms">
      <div class="dialog-header">
        <h3>选择发布平台</h3>
        <el-button class="close-button" @click="publishDialogVisible = false">
          <el-icon>
            <Close />
          </el-icon>
        </el-button>
