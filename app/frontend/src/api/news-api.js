...
export const updatePlatformConfig = async (configData) => {
  ...
  const filteredConfigData = Object.fromEntries(
    Object.entries(configData).filter(([_, value]) => value !== '' && value !== undefined && value !== null)
  );

  try {
    const response = await apiClient.post('/platforms/updateConfig', filteredConfigData);
    return response.data;
  } catch (error) {
