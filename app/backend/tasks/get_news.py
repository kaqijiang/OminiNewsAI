...
def process_feed(feed_url, new_items):
    ...
    try:
        ...
        if feed.bozo:
            # print(f"解析 RSS Feed 失败: {feed_url} - {feed.bozo_exception}")
            return

        for entry in feed.entries:
            entry_id = entry.get('id') or entry.get('guid') or entry.get('link')
            if not entry_id:
                # print(f"条目缺少唯一标识符，跳过")
                continue

            # 检查 Redis 是否已经处理过该条目
            if redis_client.exists(entry_id):
                # print(f"条目已存在，跳过: {entry_id}")
                continue
        ...
        print(f"处理 RSS Feed 时发生错误: {feed_url} - {e}")

def process_all_feeds():
    """处理所有配置的 RSS Feed"""
    # print("开始处理所有 RSS Feed。")
    new_items = []
    for feed_url in RSS_FEED_URLS:
        # print(f"正在处理 RSS Feed: {feed_url}")
        process_feed(feed_url, new_items)

    # print("所有 RSS Feed 处理完成。")
    return new_items
